...
class CommandTest extends SphinxTestCase
{
    ...
    public function testGetSetSql()
    {
        ...
        $sql2 = 'SELECT * FROM article_index';
        $command->sql = $sql2;
    ...
    public function testAutoQuoting()
    {
        ...
        $sql = 'SELECT [[id]], [[t.name]] FROM {{article_index}} t';
        $command = $db->createCommand($sql);
        $this->assertEquals("SELECT `id`, `t`.`name` FROM `article_index` t", $command->sql);
    }
    ...
    public function testPrepareCancel()
    {
        ...
        $command = $db->createCommand('SELECT * FROM article_index');
    ...
    public function testExecute()
    {
        ...
        $sql = 'SELECT COUNT(*) FROM article_index WHERE MATCH(\'dogs\')';
        $command = $db->createCommand($sql);
    ...
    public function testQuery()
    {
        ...
        // queryAll
        $rows = $db->createCommand('SELECT * FROM article_index')->queryAll();
        $this->assertEquals(2, count($rows));
        $row = $rows[1];
        $this->assertEquals(2, $row['id']);
        $this->assertEquals(2, $row['category_id']);

        $rows = $db->createCommand('SELECT * FROM article_index WHERE id=10')->queryAll();
        $this->assertEquals([], $rows);

        // queryOne
        $sql = 'SELECT * FROM article_index ORDER BY id ASC';
        ...
        $sql = 'SELECT id FROM article_index ORDER BY id ASC';
        $command = $db->createCommand($sql);
        $command->prepare();
        $this->assertEquals(1, $command->queryScalar());
    ...
    public function testInsert()
    {
        ...
        $command = $db->createCommand()->insert('rt_index', [
            'title' => 'Test title',
        ...
        $rows = $db->createCommand('SELECT * FROM rt_index')->queryAll();
    ...
    public function testReplace()
    {
        ...
        $command = $db->createCommand()->replace('rt_index', [
            'title' => 'Test title',
            'content' => 'Test content',
            'type_id' => 2,
            'category' => [1, 2],
        ...
        list($row) = $db->createCommand('SELECT * FROM rt_index')->queryAll();
    ...
    public function testUpdate()
    {
        ...
        $db->createCommand()->insert('rt_index', [
            'title' => 'Test title',
            'content' => 'Test content',
            'type_id' => 2,
            'id' => 1,
    ...
    public function testDelete()
    {
        ...
        $command = $db->createCommand()->delete('rt_index', 'id = 1');
    ...
    public function testCallSnippets()
    {
        ...
        $rows = $db->createCommand()->callSnippets('article_index', $source, $query)->queryColumn();
        ...
        $snippet = $db->createCommand()->callSnippets('article_index', $source, $query, $options)->queryScalar();
    ...
    public function testCallKeywords()
    {
        ...
        $text = 'table pencil';
        $rows = $db->createCommand()->callKeywords('article_index', $text)->queryAll();
        ...
        $text = 'table pencil';
        $rows = $db->createCommand()->callKeywords('article_index', $text, true)->queryAll();
