...
class BatchQueryResultTest extends DatabaseTestCase
{
    ...
    public function testQuery()
    {
        ...
        $result = $query->batch(2, $connection);
        ...
        $query->from('customer')->orderBy('id');
        $allRows = [];
        $batch = $query->batch(2, $connection);
        ...
        $allRows = [];
        foreach ($query->batch(2, $connection) as $rows) {
            $allRows = array_merge($allRows, $rows);
        }
        $this->assertEquals(3, count($allRows));
        ...
        $query->from('customer')->orderBy('id');
        $allRows = [];
        foreach ($query->each(100, $connection) as $rows) {
            $allRows[] = $rows;
        }
        $this->assertEquals(3, count($allRows));
        $this->assertEquals('user1', $allRows[0]['name']);
        ...
        $allRows = [];
        foreach ($query->each(100, $connection) as $key => $row) {
            $allRows[$key] = $row;
        }
        $this->assertEquals(3, count($allRows));
        $this->assertEquals('address1', $allRows['user1']['address']);
    ...
    public function testActiveQuery()
    {
        ...
        $customers = [];
        foreach ($query->batch(2, $connection) as $models) {
