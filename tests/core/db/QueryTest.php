...
class QueryTest extends DatabaseTestCase
{
    ...
    public function testSmartFilter()
    {
        ...
        $this->assertSame($query->one($connection)['name'], 0);
        ...
        $query = new Query();
        $query->from('customer');
        $query->validation(Validation::create()->key('name', Validation::int()));
        $this->assertEmpty($query->one($connection));

        // validate success
        $query = new Query();
        $query->from('customer');
        $query->validation(Validation::create()->key('name', Validation::string()));
        $this->assertNotEmpty($query->one($connection));
    }
}
